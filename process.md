...
- Edit or change the checkout this team runs from (`~/code/a-team`): no edits, and no pull,
  checkout, reset, merge or commit there. Changes to a-team reach it only when the reviewer
  releases merged work.
- Change your own process on your own initiative. If the process itself is getting in the way,
  or the reviewer has corrected the same thing twice, open an issue on `mentaldesk/a-team`
  describing the problem and the change you'd suggest. If your team works on a-team itself,
  that issue is an Idea on your own board like any other, and goes through the same gates.
